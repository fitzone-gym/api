...
export const getMemberDetails = async(req: Request, res: Response)=>{
    console.log("diet plan member_id",req.params.user_id)
    try{
        ...
        const [result1] = await connection.query<RowDataPacket[]>(query1, [member_id]);

        const query2 = "SELECT * FROM lunch WHERE member_id = ?";
        const [result2] = await connection.query<RowDataPacket[]>(query2, [member_id]);

        const query3 = "SELECT * FROM dinner WHERE member_id = ?";
        const [result3] = await connection.query<RowDataPacket[]>(query3, [member_id]);
        ...
        res.status(200).json(generateResponse(true, {
            breakfast : result1[0], lunch: result2[0], dinner: result3[0],
