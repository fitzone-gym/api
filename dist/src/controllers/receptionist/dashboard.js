...
const getCount = async (req, res) => {
    try {
        ...
        const [TotalMemberResult] = await connection.query(TotalMemberQuery);
        ...
        const [NewMemberResult] = await connection.query(NewMemberQuery);
        ...
        const [totalTrainersResult] = await connection.query(totalTrainersQuery);
        ...
        const [totalOnCallDocResult] = await connection.query(totalOnCallDocQuery);
        ...
        const [totalEventResult] = await connection.query(EventCountQuery);
        ...
        const [newTrainersResult] = await connection.query(newTrainersQuery);
        ...
        const [todayAttendenceResult] = await connection.query(todayAttendenceQuery);
        //count for attendence today in the gym
        ...
        const [currentAttendenceResult] = await connection.query(currentAttendenceQuery);
        const AnnouncemntQuery = " SELECT * FROM announcement where receiver = 'receptionist' ";
        const [dashbordannouncement] = await connection.query(AnnouncemntQuery);
        connection.release();
        ...
        res.status(200).json((0, utils_1.generateResponse)(true, counts));
    }
    catch (err) {
